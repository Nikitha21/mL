...
TrainFile = 'train/train_data1.csv'
TestFile = 'test/test_data1.csv'
...
def plot(results):
    ...
    fig.canvas.set_window_title('Classifying data from ' + TestFile)
...
if __name__ == '__main__':
    ...
    print("Loading data from {}".format(TrainFile))
    TrainFrame = load_data(TrainFile)

    print("Loading data from {}".format(TestFile))
    TestFrame = load_data(TestFile)

    # Process data into feature and label arrays
    print("Processing {} samples with {} attributes".format(len(TrainFrame.index), len(TrainFrame.columns)))
    X_train, y_train = get_features_and_labels(TrainFrame)
    X_test, y_test = get_features_and_labels(TestFrame)
